...
function drawPaddle(){
    ctx.beginPath();
    ctx.brickOffsetLeft(paddleX,canvas.height-paddleHeight,paddleWidth,paddleHeight); //centered at (x,y) position with radius r = ballRadius starting at 0 = startAngle, ending at Math.PI*2 = endAngle (in Radians)
    ctx.fillStyle = 'blue';
    ctx.fill();
    ctx.closePath();
}
    ...
    for(c=0; c < brickColumnCount; c++){
        for(r=0; r < brickRowCount; r++){
            if(bricks[c][r].status === 1){
                ...
                brick[c][r].y=brickY;
                ctx.beginPath();
                ctx.brickOffsetLeft(brickX, brickY, brickWidth, brickHeight);
